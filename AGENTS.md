...
- Не читать всю папку project-brain без необходимости.
- Для обычной задачи сначала читать:
  - AGENTS.md
  - CLAUDE.md
  - project-brain/25_team_protocol.md
  - TODO.md
  - DEVELOPMENT_PLAN.md
  - project-brain/09_last_state.md
  - project-brain/10_current_task.md
- Остальные файлы project-brain читать только если они нужны для задачи.
- Если задача про MVP, читать project-brain/03_mvp_scenario.md.
- Если задача про монетизацию, читать project-brain/04_monetization.md.
- Если задача про роли, читать project-brain/02_roles.md.
- Если задача про баги, читать project-brain/07_bugs.md.
- Делать одну маленькую задачу за раз.
- Не анализировать весь проект повторно, если достаточно прочитать последнее состояние.
- После выполненной задачи предложить обновление project-brain/09_last_state.md.
...
## Дизайн и UI — зона Claude (мягкое правило)

Если пользователь просит изменить дизайн, UI, экран, вёрстку, цвета, иконки, отступы или тексты интерфейса:

- сначала пометь: «по протоколу команды (`project-brain/25_team_protocol.md`) дизайн/UX — зона Claude» и предложи передать задачу Claude;
- выполняй сам, только если пользователь подтвердил («всё равно сделай» / «давай ты»);
- даже тогда держи премиальную локальность (см. выше) и не меняй глобальную тему/токены без отдельного разрешения.

Это не запрет, а маршрутизация: качество дизайна команда держит на стороне Claude.
